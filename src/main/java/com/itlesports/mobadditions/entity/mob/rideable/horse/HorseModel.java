...
import com.itlesports.mobadditions.entity.mob.rideable.horse.KickingAnimal;
...
public class HorseModel extends ModelBase
{
	...
	{
		HorseEntity var8 = (HorseEntity)par1Entity;
	...
	public void setLivingAnimations(EntityLiving par1EntityLiving, float par2, float par3, float par4)
	{
		...
		float var5 = this.func_110683_a(par1EntityLiving.prevRenderYawOffset, par1EntityLiving.renderYawOffset, par4);
		float var6 = this.func_110683_a(par1EntityLiving.prevRotationYawHead, par1EntityLiving.rotationYawHead, par4);
		...
		HorseEntity var10 = (HorseEntity) par1EntityLiving;
		...
		float var18 = (float)par1EntityLiving.ticksExisted + par4;
		...
		adjustRotationAnglesForKickAttack(par1EntityLiving);
	}
	...
	private void adjustRotationAnglesForKickAttack(Entity entity) {
		...
		if (cow != null) {
			...
			if (iAttackProgressCounter > 0) {
				...
				float kickAmount = MathHelper.cos((float) Math.PI * fRotationFactor) * 1.4F;

				if (cow.kickAttackLegUsed == 0) {
					backLeftLeg.rotateAngleX = kickAmount;
					backLeftShin.rotateAngleX = kickAmount;
					backLeftHoof.rotateAngleX = kickAmount;
				}
				else {
					backRightLeg.rotateAngleX = kickAmount;
					backRightShin.rotateAngleX = kickAmount;
					backRightHoof.rotateAngleX = kickAmount;
				}
			}
		}
	}
